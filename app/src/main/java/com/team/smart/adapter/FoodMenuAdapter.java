...
public class FoodMenuAdapter extends RecyclerView.Adapter<FoodMenuAdapter.Holder> {
    DecimalFormat numberComma = new DecimalFormat("###,###"); //숫자 콤마
    private ArrayList<FoodCartVO> foodList;
    private Context mCtx;

    private FoodMenuAdapter.ItemClick itemClick;
    ...
    //아이템 클릭시 실행 함수 등록 함수
    public void setItemClick(FoodMenuAdapter.ItemClick itemClick) {
        this.itemClick = itemClick;
    }

    public FoodMenuAdapter(Context context, ArrayList<FoodCartVO> pList) {
        this.mCtx = context;
        this.foodList = pList;
    }
    ...
    @NonNull
    @Override
    public FoodMenuAdapter.Holder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.food_cart_list, parent, false);
        return new FoodMenuAdapter.Holder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull FoodMenuAdapter.Holder holder, int position) {
